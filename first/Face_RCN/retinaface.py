...
class Retinaface(object):
    ...
    def encode_face_db(self, face_db_path):
        face_db = os.listdir(face_db_path)
        image_paths = []
        names = []
        for user in face_db:
            user_imgs = os.listdir(os.path.join(face_db_path, user))
            for img in user_imgs:
                image_paths.append(os.path.join(face_db_path, user, img))
                names.append(user)
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        np.save("face_encoded/{backbone}_faces.npy".format(backbone=self.facenet_backbone),face_encodings)
        np.save("face_encoded/{backbone}_names.npy".format(backbone=self.facenet_backbone),names)
